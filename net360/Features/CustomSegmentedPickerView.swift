...
//  net360
//
//  Created by Arben on 19.8.24.
//

import SwiftUI

struct CustomSegmentedPickerView1: View {
    @Binding var selectedIndex: Int
    private var titles = ["Public", "Private"]
    ...
    var body: some View {
        VStack {
            ZStack {
                HStack {
                    ForEach(self.titles.indices, id: \.self) { index in
                        Button(action: { selectedIndex = index }) {
                            DescText(self.titles[index], LayoutConstants.fontSize14, .bold, color: selectedIndex == index ? .white : .customBlueColor)
                ...
                .background(
                    Capsule().fill(Color.customBlueColor)
